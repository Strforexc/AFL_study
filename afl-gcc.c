...
static u32  cc_par_cnt = 1;         /* Param count, including argv0      cc参数  */
static u8   be_quiet,               /* Quiet mode                        静默模式  */
            clang_mode;             /* Invoked as afl-clang*?            是否使用afl-clang模式   */
...
static void find_as(u8* argv0) {
  ...
  if (afl_path) {
    ...
    tmp = alloc_printf("%s/as", afl_path);  // 动态分配内存存储

    if (!access(tmp, X_OK)) {
      as_path = afl_path;                  // 可访问即设置，return
      ck_free(tmp);
      return;
    }
...
int main(int argc, char** argv) {
  ...
  if (isatty(2) && !getenv("AFL_QUIET")) {  // 未设置静默模式，则打印
